...
class Runner:
    ...
    def setup_model(self, device):
        ...
        if self.config.MODEL.NAME == 'pivae':
        ...
        elif self.config.MODEL.NAME == 'Random':
            self.model = get_model_class(self.config.MODEL.NAME)(
                self.config.MODEL,
                self.num_neurons,
                device,
    ...
    def train_epoch(self):
        ...
        for spikes, rates, behavior, heldout_spikes, forward_spikes, forward_behavior in self.training_generator: #*
            ...
            if behavior != None and forward_behavior != None:
                behavior = torch.cat([behavior, forward_behavior], 1)
            #*
            ...
            # print("masked_spikes.shape:", masked_spikes.shape)
            ...
            if self.config.MODEL.NAME == 'pivae':
                ...
                loss = loss.mean()
            elif self.config.MODEL.NAME == 'LFADS':
                loss, _ = self.model(
                    masked_spikes,
                    mask_labels=labels,
                    rates=rates,
        ...
        if (train_cfg.DO_VAL and update % train_cfg.VAL_INTERVAL == 0):
            ...
            with torch.no_grad():
                ...
                # print("rates.shape:", rates.shape)
                if self.config.MODEL.NAME == 'Random':
    ...
    def get_rates(
        ...
        with torch.no_grad():
            ...
            layer_outputs = []
            # all_attentions = []
            for spikes, rates, behavior, heldout_spikes, forward_spikes, forward_behavior in data_generator: #*
                spikes = spikes.to(self.device)
                rates = rates.to(self.device)
                spikes_no_fp = spikes
                #*
            ...
            # Note this a list
            if self.config.MODEL.NAME in ['LFADS', 'pivae', 'lrnn', 'mint', 'PSID', 'TNDM'] :
