# Learning Under Privileged Information (LUPI) Distillation
# Author: Zhengrui Guo
...
def feature_distillation_loss(student_features, teacher_features):
    return sum(F.mse_loss(sf, tf) for sf, tf in zip(student_features, teacher_features))

def gradient_distillation_loss(student_output, teacher_output, input_data):
    student_grads = torch.autograd.grad(student_output.sum(), input_data, create_graph=True)[0]
    teacher_grads = torch.autograd.grad(teacher_output.detach().sum(), input_data, create_graph=True)[0]
    return F.mse_loss(student_grads, teacher_grads)
   
def correlation_distillation_loss(student_output, teacher_output):
    batch_size = student_output.shape[0]
    losses = []
    
    for i in range(batch_size):
        student_corr = torch.corrcoef(student_output[i].T)
        teacher_corr = torch.corrcoef(teacher_output[i].T)
        losses.append(F.mse_loss(student_corr, teacher_corr))
...
class Runner:
    ...
    def __init__(self, config=None, checkpoint_path=None):
        ...
        self.distillation_alpha = config.TRAIN.DISTILLATION_ALPHA if hasattr(config.TRAIN, 'DISTILLATION_ALPHA') else 0.5
        #*
    ...
    def save_checkpoint(
        ...
        if len(osp.split(file_name)[0]) > 0:
            ...
            model_dir = osp.join(self.config.CHECKPOINT_DIR, self.config.MODEL.NAME)
            os.makedirs(model_dir, exist_ok=True)
            if self.config.DATA.IGNORE_FORWARD:
                fp_dir = osp.join(model_dir, "noFP")
                os.makedirs(fp_dir, exist_ok=True)
            else:
                fp_dir = osp.join(model_dir, "FP")
                os.makedirs(fp_dir, exist_ok=True)
            full_path = osp.join(fp_dir, file_name)
    ...
    def load_checkpoint(self, checkpoint_path: str, *args, **kwargs) -> Dict:
        ...
        #*
        if "behavior_dim" in ckpt_dict:
            self.behavior_dim = ckpt_dict['behavior_dim']
        #*
    ...
    def load_train_val_data_and_masker(self):
        ...
        self.behavior_dim = training_set.behavior_dim
    ...
    #* Learning Under Privileged Information Distillation ******************************************
    def setup_teacher_student(self):
        # Create teacher model with privileged information
        if self.config.MODEL.NAME == 'LFADS':
            self.teacher_model = get_model_class('LFADS_Teacher')(
                self.config.MODEL,
                self.trial_length,
                self.num_neurons,
                self.device,
            ...
            self.teacher_model = get_model_class('NeuralDataTransformerTeacher')(
                self.config.MODEL,
                self.trial_length,
                self.num_neurons,
                self.device,
    ...
    def compute_distillation_loss(self, student_pred_rates, teacher_pred_rates):
        # Implement your distillation loss here
        return F.mse_loss(student_pred_rates, teacher_pred_rates)
    ...
    def train_teacher(self):
        ...
        self.logger.info(f"Teacher Model: {self.config.MODEL.NAME} with Behavior Information")
        self.model = self.teacher_model
        self.model = self.model.to(self.device)
        self.train(is_teacher = True)

    def train_student(self):
        self.logger.info("Training student model with distillation...")
        self.is_distillation = True
        if self.config.TRAIN.SOFT_DISTILLATION: #* Soft Distillation
            self.logger.info("Soft Label Distillation Initiating ...")
        elif self.config.TRAIN.FEATURE_DISTILLATION: #* Feature Distillation
            self.logger.info("Feature Distillation Initiating ...")
        elif self.config.TRAIN.CORRELATION_DISTILLATION: #* Correlation Distillation
            self.logger.info("Correlation Distillation Initiating ...")
        else:
            self.logger.info("Hard Label Distillation Initiating ...")
            
        self.logger.info(f"Student Model: {self.config.MODEL.NAME}")
        if self.config.MODEL.NAME == 'LFADS':
            self.model = get_model_class('LFADS')(
                self.config.MODEL,
                self.trial_length,
                self.num_neurons,
                self.device,
            ...
            self.model = get_model_class('NeuralDataTransformer')(
                self.config.MODEL,
                self.trial_length,
                self.num_neurons,
                self.device,
        ...
        # Reinitialize optimizer and learning rate scheduler
        # self.load_optimizer(self.model.get_factor_size())
        self.train()
        
    #**************************************************************************************************

    def train(self, checkpoint_path=None, is_teacher = False) -> None:
        ...
        #**************************************************************************
        num_hidden = self.model.get_factor_size()
        ...
        for update in range(start_updates, train_cfg.NUM_UPDATES):
            metrics = self.train_epoch(is_teacher = is_teacher)
            if metrics["done"]:
                break
        if not metrics["done"]:
    ...
    def train_epoch(self, is_teacher = False):
        r"""
        ...
        t_start = time.time()
        for spikes, rates, behavior, heldout_spikes, forward_spikes, forward_behavior in self.training_generator: #*
            spikes = spikes.to(self.device)
            rates = rates.to(self.device) if self.config.MODEL.REQUIRES_RATES else None
            #*
            if self.training_generator.dataset.has_behavior:
                behavior = behavior.to(self.device)
            else:
                behavior = None
            if self.training_generator.dataset.has_fp_behavior:
                forward_behavior = forward_behavior.to(self.device)
            else:
                forward_behavior = None
            if behavior != None and forward_behavior != None:
                behavior = torch.cat([behavior, forward_behavior], 1).to(self.device)
            #*
            ...
            if self.is_distillation and self.config.TRAIN.GRADIENT_DISTILLATION:
                masked_spikes = masked_spikes.float()
                masked_spikes.requires_grad = True
            
            if self.config.MODEL.NAME == 'LFADS':
                mlm_loss, student_pred_rates = self.model(
                    masked_spikes,
                    mask_labels=labels,
                    rates=rates,
                ...
                mlm_loss, student_pred_rates, layer_outputs, *_ = self.model(
                    masked_spikes,
                    mask_labels=labels,
                    rates=rates,
                    return_outputs=False,
            ...
            if self.is_distillation:
                with torch.no_grad():
                    if self.config.MODEL.NAME == 'LFADS':
                        teacher_mlm_loss, teacher_pred_rates = self.teacher_model(
                            masked_spikes, #! spikes or masked_spikes?
                            mask_labels=labels,
                            rates=rates,
                            return_outputs=True,
                            behavior=behavior,
                        ...
                        teacher_mlm_loss, teacher_pred_rates, teacher_layer_outputs, *_ = self.teacher_model(
                            masked_spikes, #! spikes or masked_spikes?
                            mask_labels=labels,
                            rates=rates,
                            return_outputs=True,
                ...
                if self.config.TRAIN.SOFT_DISTILLATION: #* Soft Distillation
                    # print("Soft Label Distillation Initiating ...")
                    temperature = 2.0  # You can adjust this value
                    softmax = nn.Softmax(dim=-1)
                    soft_targets = softmax(teacher_pred_rates / temperature)
                    soft_pred = softmax(student_pred_rates / temperature)
                    distillation_loss = F.kl_div(F.log_softmax(student_pred_rates / temperature, dim=-1), soft_targets, reduction='batchmean') * (temperature ** 2)
                
                elif self.config.TRAIN.FEATURE_DISTILLATION: #* Feature Distillation
                    assert self.config.MODEL.NAME == 'NeuralDataTransformer', "only NDT supports Feature Distillation!"
                    distillation_loss = F.mse_loss(student_pred_rates, teacher_pred_rates) + feature_distillation_loss(layer_outputs, teacher_layer_outputs)
                    
                elif self.config.TRAIN.CORRELATION_DISTILLATION: #* Correlation Distillation
                    # print(student_pred_rates.shape)
                    # self.logger.info(f"student_pred_rates.shape: {student_pred_rates.shape}")
                    distillation_loss = (F.mse_loss(student_pred_rates, teacher_pred_rates) + correlation_distillation_loss(student_pred_rates, teacher_pred_rates))
                    
                else: #* Hard Distillation
                    # print("Hard Label Distillation Initiating ...")
                    distillation_loss = F.mse_loss(student_pred_rates, teacher_pred_rates)
                    
                loss = self.distillation_alpha * mlm_loss.mean() + (1 - self.distillation_alpha) * distillation_loss.mean()
            else:
                loss = mlm_loss.mean()
        ...
        if (train_cfg.DO_VAL and update % train_cfg.VAL_INTERVAL == 0):
            ...
            with torch.no_grad():
                ...
                #*
                spikes, rates, behavior, heldout_spikes, forward_spikes, forward_behavior = self.validation_set.get_dataset()
                #*
                spikes = spikes.to(self.device)
                rates = rates.to(self.device)
                #*
                if self.validation_set.has_behavior:
                    behavior = behavior.to(self.device)
                else:
                    behavior = None
                if self.validation_set.has_fp_behavior:
                    forward_behavior = forward_behavior.to(self.device)
                else:
                    forward_behavior = None
                if behavior != None and forward_behavior != None:
                ...
                if self.config.MODEL.NAME == 'LFADS':
                    no_mask_loss, pred_rates = self.model(
                        spikes,
    ...
    def eval(
        ...
        with TensorboardWriter(
            ...
            with torch.no_grad():
                ...
                #*
                spikes, rates, behavior, heldout_spikes, forward_spikes, forward_behavior = test_set.get_dataset()
                #*
                spikes = spikes.to(self.device)
                rates = rates.to(self.device)
                #*
                if test_set.has_behavior:
                    behavior = behavior.to(self.device)
                else:
                    behavior = None
                if test_set.has_fp_behavior:
                    forward_behavior = forward_behavior.to(self.device)
                else:
                    forward_behavior = None
                if behavior != None and forward_behavior != None:
                ...
                if self.config.MODEL.NAME == 'LFADS':
                    loss, pred_rates = self.model(masked_spikes, mask_labels=labels, behavior=behavior) #*
                else:
                    loss, pred_rates, *_ = self.model(masked_spikes, mask_labels=labels, behavior=behavior) #*
                test_loss = loss.mean()
    ...
    def get_rates(
        ...
        #*
        self.behavior_dim = whole_set.get_dim_behavior()
        #*
        ...
        with torch.no_grad():
            ...
            layer_outputs = []
            # all_attentions = []
            for spikes, _, behavior, heldout_spikes, forward_spikes, forward_behavior in data_generator: #*
                spikes = spikes.to(self.device)
                #*
                if data_generator.dataset.has_behavior:
                    behavior = behavior.to(self.device)
                else:
                    behavior = None
                if data_generator.dataset.has_fp_behavior:
                    forward_behavior = forward_behavior.to(self.device)
                else:
                    forward_behavior = None
                if behavior != None and forward_behavior != None:
