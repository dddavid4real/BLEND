...
def make_train_input_tensors(dataset, dataset_name,
                             ...
                             include_forward_behavior=False,
                             seed=0):
    ...
    if include_forward_pred:
        ...
        #* Add forward prediction behavior if requested ************************************************
        if include_forward_behavior and include_behavior:
            behavior_source = params['behavior_source']
            behavior_field = params['behavior_field']
            fp_behavior_make_params = _prep_behavior(dataset, params.get('lag', None), fp_make_params)
            
            if behavior_source == 'data':
                fp_behavior = make_jagged_array(dataset, [behavior_field], fp_behavior_make_params, trial_mask)[0][behavior_field]
            else:
                # Adjust the behavior data to match the forward prediction period
                original_behavior = data_dict['train_behavior']
                fp_behavior = np.zeros((original_behavior.shape[0], int(fp_steps), original_behavior.shape[2]))
                for i in range(original_behavior.shape[0]):
                    fp_behavior[i] = original_behavior[i, -int(fp_steps):, :]
            
            # Apply behavior mask if necessary
            if 'behavior_mask' in params:
                if callable(params['behavior_mask']):
                    behavior_mask = params['behavior_mask'](dataset.trial_info[trial_mask])
                else:
                    behavior_mask, _ = params['behavior_mask']
                fp_behavior[~behavior_mask] = np.nan
            
            data_dict['train_behavior_forward'] = fp_behavior
        #*******************************************************************************************************************
...
#         # Retrieve behavior data from indicated source
#         if behavior_source == 'data':
#             eval_behavior = make_jagged_array(dataset, [behavior_field], behavior_make_params, trial_mask)[0][behavior_field]
#         else:
#             eval_behavior = (
#                 dataset.trial_info[trial_mask][behavior_field]
#                 .apply(lambda x: x.dt.total_seconds() if hasattr(x, "dt") else x)
...
def make_eval_input_tensors(dataset, dataset_name, 
                            ...
                            include_behavior=False,
                            seed=0):
    """Makes model evaluation input tensors with enhanced debugging for behavior data.
    Creates 3d arrays containing heldin spiking for eval trials (and heldout spiking 
    if available) and saves them as .h5 files and/or returns them in a dict
    ...
    trial_split : {'train', 'val', 'test'}, array-like, or list, optional
        The selection of trials to make the tensors with.
        It can be the predefined trial splits 'train', 'val', or 'test',
        an array-like boolean mask, or a list containing the previous two types
    update_params : dict, optional
        New parameters with which to update default dict from PARAMS
    save_file : bool, optional
        Whether to save the reshaped data to an h5 file, by default True
    return_dict : bool, optional
        Whether to return the reshaped data in a data dict, by default True
    save_path : str, optional
        Path to where the h5 output file should be saved
    include_behavior : bool, optional
        Whether to include behavioral data in output, by default False
    seed : int, optional
        Seed for random generator used for jitter
    ...
    -------
    dict of np.array
        A dict containing 3d numpy arrays of spiking data for indicated trials
    """
    assert isinstance(dataset, NWBDataset), "`dataset` must be an instance of NWBDataset"
    assert dataset_name in PARAMS.keys(), f"`dataset_name` must be one of {list(PARAMS.keys())}"
    assert isinstance(trial_split, (pd.Series, np.ndarray, list)) or trial_split in ['train', 'val', 'test'], \
    ...
    logger.info(f"Trial split: {trial_split}")
    logger.info(f"Number of trials in mask: {trial_mask.sum()}")
    ...
    if jitter is not None:
        ...
        if 'align_field' in make_params:
            ...
            make_params['start_field'] = align_jit.name
        logger.info(f"Applied jitter to alignment field: {align_jit.name}")
    ...
    if not np.any(dataset.trial_info[trial_mask].split == 'test'):
        ...
        logger.info("Created eval_spikes_heldin and eval_spikes_heldout tensors")
    else:
        eval_dict = make_stacked_array(dataset, [spk_field], make_params, trial_mask)
        data_dict = {
            'eval_spikes_heldin': eval_dict[spk_field],
        }
        logger.info("Created eval_spikes_heldin tensor")
    ...
    if include_behavior:
        logger.info(f"Processing behavior data for {dataset_name}")
        
        behavior_source = params['behavior_source']
        behavior_field = params['behavior_field']
        
        logger.info(f"Behavior source: {behavior_source}")
        logger.info(f"Behavior fields: {behavior_field if isinstance(behavior_field, list) else [behavior_field]}")
        
        # Special handling for DMFC RSG dataset
        if dataset_name == 'dmfc_rsg':
            logger.info("Using special handling for DMFC RSG behavior data")
            try:
                # Convert trial_info data into numpy array with proper handling of each field
                behavior_data = []
                trial_info_subset = dataset.trial_info[trial_mask]
                
                for field in behavior_field:
                    logger.info(f"Processing field: {field}")
                    if field not in trial_info_subset.columns:
                        raise KeyError(f"Field {field} not found in trial_info")
                    
                    field_data = trial_info_subset[field]
                    if hasattr(field_data.iloc[0], 'dt'):
                        logger.info(f"Converting timedelta field {field} to seconds")
                        field_data = field_data.dt.total_seconds()
                    field_data = field_data.astype(float)
                    behavior_data.append(field_data)
                
                eval_behavior = np.column_stack(behavior_data)
                logger.info(f"Successfully created behavior array with shape: {eval_behavior.shape}")
                
                # Apply behavior mask if specified
                if 'behavior_mask' in params:
                    logger.info("Applying behavior mask")
                    if callable(params['behavior_mask']):
                        behavior_mask = params['behavior_mask'](trial_info_subset)
                        logger.info(f"Number of trials after mask: {behavior_mask.sum()}")
                    else:
                        behavior_mask, _ = params['behavior_mask']
                    eval_behavior[~behavior_mask] = np.nan
                
                data_dict['eval_behavior'] = eval_behavior
                logger.info("Successfully added DMFC RSG behavior data to data_dict")
                
            except Exception as e:
                logger.error(f"Error processing DMFC RSG behavior data: {str(e)}")
                logger.warning("Skipping behavior data due to processing error")
                
        else:
            # Standard behavior processing for other datasets
            behavior_make_params = _prep_behavior(dataset, params.get('lag', None), make_params)
            logger.info(f"Behavior make params: {behavior_make_params}")
            
            try:
                if behavior_source == 'data':
                    logger.info("Extracting behavior data from data source")
                    eval_behavior = make_jagged_array(dataset, [behavior_field], behavior_make_params, trial_mask)[0][behavior_field]
                else:  # behavior_source == 'trial_info'
                    logger.info("Extracting behavior data from trial_info")
                    eval_behavior = (
                        dataset.trial_info[trial_mask][behavior_field]
                        .apply(lambda x: x.dt.total_seconds() if hasattr(x, "dt") else x)
                        .to_numpy()
                ...
                logger.info(f"Successfully extracted behavior data with shape: {eval_behavior.shape}")
                ...
                data_dict['eval_behavior'] = eval_behavior
                logger.info("Successfully added eval_behavior to data_dict")
                
            except Exception as e:
                logger.error(f"Error processing behavior data: {str(e)}")
                logger.warning("Skipping behavior data due to processing error")
