...
# Added configs for implementation of priviledged knowledge distillation
import math
...
class NeuralDataTransformer(nn.Module):
    ...
    def __init__(self, config, trial_length, num_neurons, device, max_spikes, behavior_dim): #!dim_behavior
        super().__init__()
        self.config = config
        ...
        #! for dmfc_rsg
        # if config.LINEAR_EMBEDDER:
        #     # Similar padding strategy as teacher model but without behavior dimension
        #     neural_target_dim = num_neurons  # Just the neurons
        #     total_target_dim = ((neural_target_dim + config.NUM_HEADS - 1) 
        #                     // config.NUM_HEADS) * config.NUM_HEADS  # rounds up to next multiple of NUM_HEADS
        #     pad_amount = total_target_dim - neural_target_dim
            
        #     # Create embedder with padding
        #     self.embedder = nn.Sequential(
        #         nn.Linear(self.num_neurons, total_target_dim)  # Project to dimension divisible by NUM_HEADS
        #     )
        #     # Update num_input to reflect new dimension
        #     self.num_input = total_target_dim
        # elif config.EMBED_DIM == 0:
        #     # If using identity, still need to ensure dimension is divisible by NUM_HEADS
        #     total_target_dim = ((num_neurons + config.NUM_HEADS - 1) 
        #                     // config.NUM_HEADS) * config.NUM_HEADS
        #     self.embedder = nn.Sequential(
        #         nn.Linear(num_neurons, total_target_dim)
        #     )
        #     self.num_input = total_target_dim
        # else:
        #     self.embedder = nn.Sequential(
    ...
    def forward(self, src, mask_labels, **kwargs):
        # print(src.size()) 
        #* scr shape: bs x (time + forward_steps) x neurons
        #* behavior shape: bs x time x behavior_dim
        #* currently decide not to use forward_step behaviors
        #* during validation and testing, behavior is None -> if so, ignore the fusion modules of behavior-guidance
        behavior = kwargs['behavior'] 
        kwargs.pop('behavior')
        
        # src = src.permute(1, 0, 2) # t x b x n
        if self.config.LINEAR_EMBEDDER:
            src = src.float().permute(1, 0, 2) # t x b x n
        else:
            src = src.permute(1, 0, 2) # t x b x n
...
class NeuralDataTransformerTeacher(nn.Module):
    r"""
        Transformer encoder-based dynamical systems decoder. Trained on MLM loss. Returns loss and predicted rates.
    """
    ...
    def __init__(self, config, trial_length, num_neurons, device, max_spikes, behavior_dim): #!dim_behavior
        ...
        if config.LINEAR_EMBEDDER:
            ...
            #! modification for dmfc_rsg
            # neural_target_dim = 59  # 54 + 5
            # total_target_dim = ((neural_target_dim + config.NUM_HEADS - 1) 
            #                 // config.NUM_HEADS) * config.NUM_HEADS  # rounds up to next multiple of NUM_HEADS
            # pad_amount = total_target_dim - neural_target_dim
            # # Split the padding between neural and behavior embeddings
            # self.embedder = nn.Sequential(
            #     nn.Linear(self.num_neurons, self.num_neurons + pad_amount)  # 54 -> 59
            # )
            # self.behavior_embedder = nn.Sequential(
            #     nn.Linear(self.behavior_dim, self.behavior_dim)  # 5 -> 5
            # )
            # # Update num_input to reflect new total dimension
            # self.num_input = total_target_dim - self.behavior_dim
            #!
        elif config.EMBED_DIM == 0:
            self.embedder = nn.Identity()
        ...
        #! Learnable Temporal expansion for DMFC_RSG
        # Add temporal expansion module
        # self.behavior_temporal_expansion = nn.Sequential(
        #     nn.Linear(behavior_dim, behavior_dim * 2),
        #     nn.ReLU(),
        #     nn.Linear(behavior_dim * 2, behavior_dim)
        # )
        # self.behavior_temporal_pos = PositionalEncoding(config, trial_length, behavior_dim, device)
        #!

        self.scale = math.sqrt(self.num_input)
        self.behavior_scale = math.sqrt(self.behavior_dim) #*
        #*
        self.num_input = self.num_input + self.behavior_dim
        #*
        self.pos_encoder = PositionalEncoding(config, trial_length, self.num_input, device) 
        self._init_transformer()
    ...
    # def expand_behavior_temporally(self, behavior, time_steps):
    #     """
    #     Expands behavior from (bs x behavior_dim) to (bs x time x behavior_dim)
    #     using learned transformations
    #     """
    #     # Ensure behavior is float
    #     behavior = behavior.float()  # Add this line
        
    #     bs, dim = behavior.shape
        
    #     # Initial expansion
    #     expanded = behavior.unsqueeze(1).expand(-1, time_steps, -1)  # bs x time x behavior_dim
        
    #     # Apply learned transformation
    #     transformed = self.behavior_temporal_expansion(expanded)
        
    #     # Add positional information
    #     if hasattr(self, 'behavior_temporal_pos'):
    #         transformed = transformed.permute(1, 0, 2)  # time x bs x dim
    #         transformed = self.behavior_temporal_pos(transformed)
    #         transformed = transformed.permute(1, 0, 2)  # bs x time x dim
    ...
    def forward(self, src, mask_labels, **kwargs):
        ...
        #!
        # if len(behavior.shape) == 2:  # discrete: bs x behavior_dim
        #     # Expand behavior temporally
        #     behavior = behavior.float()
        #     time_steps = src.size(1)  # Get temporal dimension from src
        #     behavior = behavior.unsqueeze(1).expand(-1, time_steps, -1) #! simple extension
        #     # behavior = self.expand_behavior_temporally(behavior, time_steps) #! learnable extension
        #!
        
        if self.config.LINEAR_EMBEDDER:
            behavior = behavior.float().permute(1,0,2) # t x b x n
        else:
            behavior = behavior.permute(1,0,2) # t x b x n
            
        behavior = self.behavior_embedder(behavior) * self.behavior_scale
        # behavior = self.instance_norm_behavior(behavior.permute(1, 2, 0)).permute(2, 0, 1) #*
        # behavior = self.behavior_encoder(behavior) * math.sqrt(self.behavior_dim) #*
        # behavior = self.behavior_encoder(behavior) #*
        # behavior = self.behavior_pe(behavior)
        if self.config.LINEAR_EMBEDDER:
            src = src.float().permute(1, 0, 2) # t x b x n
        else:
            src = src.permute(1, 0, 2) # t x b x n
        ...
        src = torch.cat([src, behavior], dim=-1) #*
