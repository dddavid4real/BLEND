...
# Added configs for including behavior information
...
class SpikesDataset(data.Dataset):
    ...
    def __init__(self, config, filename, mode=DATASET_MODES.train, logger=None):
        ...
        # self.datapath = osp.join("../data", filename) #!
        self.datapath = osp.join("/home/gzr/code/BLEND/data", filename)
        ...
        self.has_fp_behavior = False
        #*
        if len(split_path) == 1 or split_path[-1] == "h5":
            #*
            spikes, rates, behavior, heldout_spikes, forward_spikes, forward_behavior = self.get_data_from_h5(mode, self.datapath)
            #*
            spikes = torch.tensor(spikes).long()
            #*
            if behavior is not None:
                self.has_behavior = True
                behavior = torch.tensor(behavior).long()
            if forward_behavior is not None:
                self.has_fp_behavior = True
                forward_behavior = torch.tensor(forward_behavior).long()
            #*
        ...
        self.num_trials, _, self.num_neurons = spikes.size() # num_trials x time x num_neurons
        #*
        # if self.config.TRAIN_FILENAME.split(".")[0] == 'dmfc_rsg':
        if 'dmfc_rsg' in self.config.TRAIN_FILENAME:
             _, self.behavior_dim = behavior.size() if self.has_behavior else (None, None) #* dmfc_rsg only have two-dim behavior??
        else:
            _, _, self.behavior_dim = behavior.size() if self.has_behavior else (None, None, None) # num_trials x time x behavior_dim
        #*
        ...
        #*
        self.behavior = self.batchify(behavior) if self.has_behavior else torch.zeros_like(spikes)
        self.forward_behavior = self.batchify(forward_behavior) if self.has_fp_behavior else torch.zeros_like(spikes)
        #*
        ...
        if config.DATA.OVERFIT_TEST: # default False
            if self.logger is not None:
                self.logger.warning("Overfitting..")
            self.spikes = self.spikes[:2]
            self.rates = self.rates[:2]
            self.num_trials = 2
        elif hasattr(config.DATA, "RANDOM_SUBSET_TRIALS") and config.DATA.RANDOM_SUBSET_TRIALS < 1.0 and mode == DATASET_MODES.train: # default No
            if self.logger is not None:
    ...
    def __getitem__(self, index):
        ...
        """
        return (
            self.spikes[index], # time x neuron (channel)
            None if self.rates is None else self.rates[index],
            #*
            None if self.behavior is None else self.behavior[index], # time x behavior_dim
            #*
            ...
            None if self.forward_behavior is None else self.forward_behavior[index],
        )

    def get_dataset(self):
        # return self.spikes, self.rates, self.heldout_spikes, self.forward_spikes
        #*
        return self.spikes, self.rates, self.behavior, self.heldout_spikes, self.forward_spikes, self.forward_behavior
        #*
    ...
    def get_data_from_h5(self, mode, filepath):
        ...
        with h5py.File(filepath, 'r') as h5file:
            ...
            if f'eval_{NLB_KEY}_heldin' in h5dict: # NLB data, presumes both heldout neurons and time are available
                ...
                if f'train_{NLB_KEY}_heldin_forward' in h5dict:
                    ...
                    train_data_fp = None
                    train_data_heldout_fp = None
                    train_data_all_fp = None #!
                ...
                if f'eval_{NLB_KEY}_heldin_forward' in h5dict:
                    ...
                    ) if train_data_fp is not None else None #!
                #*
                # train_behavior = h5dict['train_behavior'].astype(np.float32).squeeze()
                train_behavior = get_key('train_behavior')
                val_behavior = None if 'eval_behavior' not in h5dict else get_key('eval_behavior')
                train_fp_behavior = None if 'train_behavior_forward' not in h5dict else get_key('train_behavior_forward')
                # val_fp_behavior = None if 'eval_behavior_forward' not in h5dict else get_key('eval_behavior_forward')
                val_fp_behavior = np.zeros((valid_data.shape[0], train_data_fp.shape[1], train_fp_behavior.shape[2]), dtype=np.float32) if train_fp_behavior is not None else None
                ...
                if mode == DATASET_MODES.train:
                    ...
                    #*
                    return train_data, None, train_behavior, train_data_heldout, train_data_all_fp, train_fp_behavior
                    #*
                elif mode == DATASET_MODES.val:
                    # return valid_data, None, valid_data_heldout, valid_data_all_fp
                    #*
                    return valid_data, None, val_behavior, valid_data_heldout, valid_data_all_fp, val_fp_behavior
                    #*
        ...
        if mode == DATASET_MODES.train:
            #*
            return train_data, train_rates, train_behavior, None, None
            #*
        elif mode == DATASET_MODES.val:
            #*
            return valid_data, valid_rates, None, None, None
            #*
            ...
            #*
            return file_data, merged_rates if self.has_rates else None, None, None, None
            #*
