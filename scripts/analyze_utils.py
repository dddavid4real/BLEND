...
# Added configs for including behavior-guided data
import os
...
def setup_dataset(runner, mode):
    ...
    if behavior is not None:
        behavior = behavior.to(runner.device)
    #*
    ...
    if forward_behavior is not None:
        forward_behavior = forward_behavior.to(runner.device)
    # return spikes.to(runner.device), rates.to(runner.device), heldout_spikes, forward_spikes
    #*
    return spikes.to(runner.device), rates.to(runner.device), behavior, heldout_spikes, forward_spikes, forward_behavior
    #*
...
def init_by_ckpt(ckpt_path, mode=DATASET_MODES.val):
    ...
    #*
    spikes, rates, behavior, heldout_spikes, forward_spikes, forward_behavior = setup_dataset(runner, mode)
    return runner, spikes, rates, behavior, heldout_spikes, forward_spikes, forward_behavior
    #*
