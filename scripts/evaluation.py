...
#* Evaluation functions for synthetic datasets
def evaluate_temporal_scales(rates, behavior, n_groups):
    """
    Evaluates how well the model captures different temporal scales in hierarchical data
    ...
    behavior : np.ndarray
        Behavioral data
    n_groups : int
        Number of temporal scale groups
        
    Returns
    -------
    float
        Score measuring temporal scale separation (0-1)
    """
    # Split neurons into groups
    n_neurons = rates.shape[-1]
    neurons_per_group = n_neurons // n_groups
    
    # Calculate autocorrelation at different lags for each group
    autocorr_scores = []
    for i in range(n_groups):
        start_idx = i * neurons_per_group
        end_idx = start_idx + neurons_per_group
        group_rates = rates[..., start_idx:end_idx]
        
        # Calculate mean autocorrelation at characteristic timescale
        lag = (i + 1) * 5  # Increasing lags for different groups
        autocorr = np.mean([np.corrcoef(x[:-lag], x[lag:])[0,1] 
                           for x in group_rates.reshape(-1, group_rates.shape[-1])])
        autocorr_scores.append(autocorr)
    
    # Score based on how well groups maintain their expected temporal scaling
    return np.mean([s2 - s1 for s1, s2 in zip(autocorr_scores[:-1], autocorr_scores[1:])])

def evaluate_population_structure(rates, n_groups):
    """
    Evaluates how well the model preserves population structure in complex data
    ...
    -------
    float
        Score measuring population structure preservation (0-1)
    """
    # Flatten time dimension
    flat_rates = rates.reshape(-1, rates.shape[-1])
    
    # Calculate correlation matrix
    corr_matrix = np.corrcoef(flat_rates.T)
    
    # Split neurons into groups
    n_neurons = rates.shape[-1]
    neurons_per_group = n_neurons // n_groups
    
    # Calculate mean within-group and between-group correlations
    within_group_corr = []
    between_group_corr = []
    ...
    for i in range(n_groups):
        ...
        group_corr = corr_matrix[start_idx:end_idx, start_idx:end_idx]
        within_group_corr.append(np.mean(group_corr[np.triu_indices(group_corr.shape[0], k=1)]))
        
        # Between-group correlations
        for j in range(i + 1, n_groups):
            other_start = j * neurons_per_group
            other_end = other_start + neurons_per_group
            between_corr = corr_matrix[start_idx:end_idx, other_start:other_end]
            between_group_corr.append(np.mean(between_corr))
    
    mean_within = np.mean(within_group_corr)
    mean_between = np.mean(between_group_corr)
    
    # Normalize to 0-1 range
    score = (mean_within - mean_between) / (mean_within + mean_between + 1e-10)
    return max(0, min(1, score))

def evaluate(test_annotation_file, user_submission_file):
    """
    Runs evaluation as it would be run on EvalAI servers. Now includes synthetic dataset evaluation.
    ...
    if isinstance(test_annotation_file, str):
        target_data = h5py.File(test_annotation_file, "r")
    else:
        target_data = test_annotation_file
    if isinstance(user_submission_file, str):
    ...
    # First evaluate synthetic datasets if present
    synthetic_datasets = ['simple', 'hierarchical', 'complex']
    for dataset in synthetic_datasets:
        ...
        # Calculate bits per spike (common metric)
        result_dict["co-bps"] = float(bits_per_spike(eval_rates_heldout, eval_spikes_heldout))
        
        # Extract train rates for behavioral decoding
        ...
        # Calculate velocity R2 (common metric)
        result_dict["vel R2"] = velocity_decoding(
            train_rates,
            train_behavior,
            np.full((train_rates.shape[0], 1), True),
            eval_rates,
            eval_behavior,
            np.full((eval_rates.shape[0], 1), True)
        )
        
        # Dataset-specific metrics
        if dataset == 'hierarchical':
            result_dict['temporal_scale_sep'] = evaluate_temporal_scales(
                eval_rates_heldout,
                eval_behavior,
                n_groups=3
            )
        elif dataset == 'complex':
            result_dict['pop_structure'] = evaluate_population_structure(
                eval_rates_heldout,
                n_groups=5
            )
    ...
    # Then evaluate original datasets
    for dataset in [
        "mc_maze",
        "mc_rtt",
        "area2_bump",
